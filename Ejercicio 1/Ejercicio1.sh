...
if [ $# != 1 ]; then
	echo "Error de llamada. Se debe recibir 1 solo parametro"
	exit
fi
case $1 in
	-h|-\?|-help) 
...
if [ $? != 0 ]
then
	echo "El archivo enviado como parametro no es un archivo plano"
	exit
fi

cp $1 $1.bak
ARCH1=/tmp/`basename $1`.$$
sed 's/[A-Z]*/\L&/g' $1 > $ARCH1 #Pone la primer letra de cada linea en mayuscula
...
# 3. La variable $1 es una variable especial que retorna el primer parametro recibido por la consola de ejecución,
#   la variable $# es una variable especial que devuelve la cantidad de parametros enviados por la consola de ejecución.
#	Mientras que la variable $? devuelve el valor de retorno del último comando ejecutado, donde el valor 0 es bueno.

#	Existen más variables especiales que podemos utilizar para referenciar otros objetos o parametros. Algunas de ellas son:
	
#	$0 -> nos devuelve el nombre del archivo ejecutable
#	$* -> linea completa de la llamada. Contiene todos los argumentos
#	$@ -> lista de todos los parametros recibidos por la consola de ejecución (menos $0)
...
# 6. Si ejecuto el script con un archivo con espacios en su nombre, da error debido a que toma el espacio como el fin de un parametro
#   y la siguiente palabra la utiliza como un nuevo parametro.

#	La corrección que le hariamos sería modificar el primer if del script que se encarga de verificar que recibe un solo parametro,
#	la solución que proponemos seria concatenar los parametros al inicio del script, por lo tanto ya no tendría más espacios.